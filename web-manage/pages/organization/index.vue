<template>
  ...
  <div class="q-pa-md">
    ...
    >
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          <q-td auto-width class="q-gutter-sm width-btn">
            <q-btn
              dense
              color="primary"
              size="sm"
              icon="list"
              label="คอร์สและผู้อบรม"
              :to="`/organization/${props.row.id}/user`"
            >
              <q-tooltip>คลิกไปที่คอร์สและรายการผู้อบรม</q-tooltip>
            </q-btn>
          ...
          <q-td v-for="col in props.cols" :key="col.name" :props="props">
            ...
            <div v-else-if="col.name === 'logo'">
              <img
                v-if="col.value && col.value != 'logo-default.png'"
                :src="col.value"
                style="max-height: 50px"
                :alt="col.value"
                srcset=""
              />
              <q-icon v-else name="business" size="lg" color="grey" />
            </div>
  ...
  <AddOrg v-model:modal="modal" :data="dataRow" :fetch-lists="fetchLists" />
</template>
...
async function fetchLists() {
  ...
  const response: any = await useApi(
    "GET",
    `/customer?page=${pagination.value.page}&pageSize=${pagination.value.rowsPerPage}&status=${type.value}&keyword=${filter.value}&sortBy=${pagination.value.sortBy}&descending=${pagination.value.descending}`
  );
  if (response.data) {
...
function onRemove(id: string) {
  $q.dialog({
    ...
    $q.loading.show();
    const response: any = await useApi("DELETE", `/customer/${id}`);
    if (response.data) {
      $q.loading.hide();
      $q.notify({
